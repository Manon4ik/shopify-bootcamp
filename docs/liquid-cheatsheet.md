...
##capture - перезаписываемая переменная в зависимости от условия

##if - логическая проверка

##unless - выполнения чего либо пока соблюдается условие. Аналог While.

##for - цикл перебора значений

##render - способ (функция) рендера блоков и тд. которая использует переисполльзуэмые блоки, секци, снипеты и передает значения в них. Похоже не функцию.
